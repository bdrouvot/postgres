...
SELECT (pg_identify_object_as_address(classid, objid, objsubid)).*
    ...
    ORDER BY 1, 2, 3;
SELECT *
    FROM (SELECT (pg_identify_object(classid, objid, objsubid)).*
          FROM (SELECT DISTINCT classid, objid, objsubid FROM deps_tree)) s
    ORDER BY type, schema, name COLLATE "C", identity COLLATE "C";
