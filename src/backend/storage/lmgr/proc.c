...
ProcSleep(LOCALLOCK *locallock)
{
	...
	do
	{
		...
		if (deadlock_state != DS_NOT_YET_CHECKED)
		{
			...
			/*
			 * Increment the lock statistics counters. That's OK from an
			 * overhead point of view as we are already stuck behind a lock
			 * for more than the deadlock timeout and we can reuse its wait
